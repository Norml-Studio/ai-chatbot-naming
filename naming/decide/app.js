...
async function checkBridge() { try { const r = await fetch("/api/health", { cache: "no-store" }); if (!r.ok) throw new Error(); bridgeAvailable = true; elements.bridgeStatus.textContent = "Local Codex bridge ready"; } catch { bridgeAvailable = false; elements.bridgeStatus.textContent = location.hostname === "localhost" || location.hostname === "127.0.0.1" ? "Bridge unavailable" : "Local generation is available at localhost"; } }
